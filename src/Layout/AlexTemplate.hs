-- The only way alex allows specifying a custom wrapper is by overriding the entire template directory, which means we
-- also need to provide a template. Getting it dynamically during build adds a lot of complexity, so we copied it from:
--   ~/.cabal/store/ghc-8.10.7/lx-3.2.7.4-d2d6ba6b/share/AlexTemplate.hs.
