...
normalizePres (OverlayP id d (pres:press))             = OverlayP id d (normalizePres pres : press)
...
locateTreePres' location (0:path) (OverlayP id _ press@(pres:_)) = locateTreePres' location path (index "PresUtils.locateTreePres'" press 0)              
...
isEditableTreePres' editable (0:path) (OverlayP id _ press@(pres:_)) = isEditableTreePres' editable path (index "PresUtils.isEditableTreeTreePres'" press 0)              
...
leftWidthPres (OverlayP _ _ (pres:press)) = leftWidthPres pres
...
rightWidthPres (OverlayP _ _ (pres:press)) = rightWidthPres pres
...
topHeightPres (OverlayP _ _ (pres:press)) = topHeightPres pres
...
bottomHeightPres (OverlayP _ _ (pres:press)) = bottomHeightPres pres
...
pathFromXYPres (x,y,b) (OverlayP id _ (pres:press))  = 0 `consPathP` pathFromXYPres (x,y,b) pres
...
stringFromPres' (OverlayP _ _ (pres:press)) = stringFromPres' pres
...
pathToLeftmostLeaf (OverlayP _ _ (pres:_))   = case pathToLeftmostLeaf pres of -- only navigate in head of overlay
                                            Nothing  -> Nothing
                                            Just pth -> Just $ 0 : pth
...
pathToRightmostLeaf (OverlayP _ _ (pres:_)) = case pathToRightmostLeaf pres of -- only navigate in head of overlay
                                            Nothing  -> Nothing
                                            Just pth -> Just $ 0 : pth
...
selectTree (p:path) (OverlayP _ _ press)        = selectTree path (index "PresUtils.selectTree" press p)
...
containsColPres (0:path) (OverlayP id _ press@(pres:_)) = containsColPres path (index "PresUtils.containsColPres" press 0)     
...
focusIsOnGraphPres (0:path) (OverlayP _ _ (pres:press)) = focusIsOnGraphPres path pres
